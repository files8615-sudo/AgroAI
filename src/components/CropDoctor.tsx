  ...
  return (
    <div className="space-y-6 h-full pb-20">
      <div className="flex items-center justify-between">
        <h2 className="text-2xl font-bold text-gray-800">{t('scan_crop')}</h2>
        <span className="text-xs font-medium bg-agro-100 text-agro-700 px-3 py-1 rounded-full">AI Powered</span>
      </div>
      ...
      {!image ? (
        ...
        >
          ...
          <p className="font-medium text-lg">Tap to Upload Photo</p>
          <p className="text-sm mt-2">Take a clear picture of the plant</p>
          <input 
            type="file" 
            ref={fileInputRef} 
                ...
                <div className="grid gap-4">
                  <div className="bg-white p-5 rounded-xl border border-green-100 shadow-sm">
                    <h4 className="font-bold text-green-700 mb-4 flex items-center gap-2">
                      <Leaf size={18} /> {t('treatment')}
                    </h4>
                    <ul className="space-y-4">
                      {result.naturalTreatments.map((t, i) => (
                        <li key={i} className="text-sm">
                          <p className="font-semibold text-gray-800">{t.name}</p>
                          <p className="text-gray-600 mt-0.5">{t.instruction}</p>
                        </li>
                      ))}
                    </ul>
                  </div>
