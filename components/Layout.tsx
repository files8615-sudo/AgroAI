...
export const Layout: React.FC<LayoutProps> = ({ children, currentView, onNavigate, onLogout }) => {
  ...
  const NavItem = ({ view, icon: Icon, labelKey }: { view: AppView; icon: any; labelKey: string }) => {
    ...
    return (
      ...
      >
        ...
        <span className="text-[10px] font-medium mt-1">{t(labelKey)}</span>
      </button>
    );
  };
  ...
  return (
    <div className="min-h-screen bg-earth-50 flex flex-col max-w-md mx-auto shadow-2xl overflow-hidden relative">
      <header className="bg-white px-4 py-3 flex justify-between items-center sticky top-0 z-20 shadow-sm border-b border-gray-100">
        <h1 className="text-xl font-extrabold text-agro-800 tracking-tight">{t('app_title')}</h1>
      ...
      <nav className="bg-white border-t border-gray-100 flex justify-between items-center px-2 sticky bottom-0 pb-safe z-30">
        <NavItem view={AppView.DASHBOARD} icon={LayoutDashboard} labelKey="nav_home" />
        <NavItem view={AppView.DIAGNOSIS} icon={Camera} labelKey="nav_doctor" />
        <NavItem view={AppView.NOTES} icon={BookOpen} labelKey="nav_notes" />
        <NavItem view={AppView.EXPERT} icon={MessageCircle} labelKey="nav_expert" />
      </nav>
    </div>
  );
};